  ...
  $.fn.birthdaypicker = function( options ) {
    ...
    return this.each(function() {
      ...
      if (settings["hiddenDate"]) {
        $("<input type='hidden' name='" + settings["fieldName"] + " '   id='"+settings["fieldId"]+"'/>    value='"+hiddenDate+"'").appendTo($fieldset);
        	
      }
      var hiddenAge;
      //创建隐藏的age文本框
      if (settings["hiddenAge"]) {
          $("<input type='hidden' name='" + settings["fieldAgeName"] + " '   id='"+settings["fieldAgeId"]+"'/>    value='"+hiddenAge+"'").appendTo($fieldset);
        }
      ...
      $fieldset.change(function() {
        ...
        //根据生日得出年龄
         ...
         if(todayMonth*1-selectedMonth*1<0)
         {
        	 hiddenAge = (todayYear*1-selectedYear*1)-1;
         }
         else
         {
           if(todayDay-selectedDay>=0)
           {//alert(thisDay+'-'+brithd+"_ddd");
        	  hiddenAge = (todayYear*1-selectedYear*1);
           }
           else
           {
        	   hiddenAge = (todayYear*1-selectedYear*1)-1;
      ...
      // update the hidden date
         if ((selectedYear * selectedMonth * selectedDay) != 0) {
           ...
           $(this).find('#'+settings["fieldAgeId"]).val(hiddenAge);
