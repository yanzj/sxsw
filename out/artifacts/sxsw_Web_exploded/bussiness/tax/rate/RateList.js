...
function my_initGrid(){
	window['g']=$("#maingrid").ligerGrid({
		checkbox: true,
		rowHeight:22,
		url:"RateQueryList.action",
		columns: [	
		          
		    { display: '行政区', name: 'AREANAME',  width:"20%" },
			{ display: '费率', name: 'RATENAME',  width:"20%" },
   ...
   */
function  itemclick(){
	var url = "bussiness/tax/rate/RateAdd.jsp";
	winOpen(url,'添加费率',500,300,'添加','取消',function(data){
       	$.ajax({
			url:"RateAdd.action", 
			data:data,
			dataType:"json", 
...
function  itemedit(){
	...
	var url = "bussiness/tax/rate/RateEdit.jsp?id='"+id+"'";
	winOpen(url,'修改费率',500,300,'修改','取消',function(data){
       	$.ajax({
			url:"RateUpdate.action", 
			data:data,
			dataType:"json", 
			type:"post",
			success:function (mm) {
